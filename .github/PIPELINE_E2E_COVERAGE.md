...
- **MetaxPath preflight (cross-workflow).** MetaxPath is a *private* package (not in this repo's `pipelines/`), so it isn't part of the SLURM E2E. Its first check lives in the **Alma install E2E** (`install-profile-alma.yml` → `scripts/assert-install-profile-applied.mjs`): when the `ci-runner` profile enables metaxpath (admin-configurable), the assert now verifies the package is **installed** (`pipelines/metaxpath/manifest.json`) and its **DB params file exists on disk** (the configured `paramsFile`). A full metaxpath *run* on the SLURM E2E remains planned (needs the private package + DB staged on the runner).
