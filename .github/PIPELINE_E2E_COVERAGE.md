...
| reads-qc              | ⚠️     | ⚠️     | study  | `completes` gate (run `completed`, outputs retrievable)                      | —            | per-process `seqkit`/`python` conda (shared cacheDir like fastqc) | **wired warn-only** (SLURM E2E) — **confirmed it hits the local `completed`→`running` flip** (`status is running, expected completed`), same signature as metaxpath; promote to hard once that flip is fixed |
...
| metaxpath             | —     | —     | study  | taxonomy/path results                                                        | —            | **MetaXpath DB bundle** (private pkg; **no GTDB** — mag-only) | preflight wired (Alma install E2E); **run wired (warn-only)** on the Gemma study, `completes`. **Install/enable + study run now work** — it executes the taxonomy classification (~3 min) but hits the local `completed`→`running` flip (`status is running, expected completed`), the same bug deferring reads-qc |
...
- **reads-qc** — study target; per-process `seqkit`/`python` conda. **Now wired (warn-only)** in the SLURM E2E (a study-target `--pipeline-id reads-qc` step, local + SLURM, `completes` gate). The warn-only framing is deliberate: the real-data Gemma run surfaced a local-run `completed`→`running` re-sync flip (distinct from the queue-side bug #5 guard), and this step **confirmed the flip still bites on synthetic data** (`status is running, expected completed`, no poll timeout — the run reaches `completed`, then a re-sync flips it back) **without** reding the suite. **Same signature as metaxpath**, so the local `completed`→`running` flip is now a *shared* metaxpath + reads-qc blocker — **fixing that one bug unblocks both**; then promote reads-qc to a hard step + a read-field/artifact spec (beyond `completes`; `readCount`/`avgQuality` are now exposed by the run-GET select).
...
| reads-qc          | ✅ read QC/metrics             | ⛔ deferred — the local study run reached `completed` then a re-sync flipped it back to `running` (the local `completed→running` flip; **also reproduced in the SLURM E2E**, shared with metaxpath; distinct from the queue-side bug #5 guard) |
...
| metaxpath         | ✅ taxonomy/path (the point)   | **wired (warn-only)** — `run_gemma_warn metaxpath` on the `gemma-nanopore-metaxpath` **study** (`completes`). **Install + study run now work** — it executes the taxonomy classification (~3 min) but hits the local `completed`→`running` flip (`status is running`), the same bug deferring reads-qc; warn-only. **GTDB not needed** (mag-only) |
...
Follow-ups: **fix the local `completed`→`running` flip** — now the single shared blocker for both metaxpath (runs the study taxonomy, then flips) and reads-qc (same signature); it is distinct from the queue-side bug #5 guard (the local/no-trace finalize path appears to lack an equivalent terminal guard), and fixing it turns both warn-only runs green; pin the **trace-path writeback flake** (fastqc read-field merge + run-scoped summaries don't ingest when the pipeline-monitor finalizes — cf. bug #4); enable study-demo-report on the profile to cover it on Gemma; stage **GTDB for mag** (not metaxpath — metaxpath needs only its own DB bundle).
...
**Surfaced this round — fix pending** (each mitigated with warn-only so CI stays green):

6. **Local `completed`→`running` re-sync flip** — distinct from bug #5 (which guarded the with-trace/queue branch): on the **local / no-trace** finalize path a run reaches `completed`, then a re-sync flips it back to `running`, and the writeback gate sees `status is running, expected completed` (no poll timeout). Caught on **both** metaxpath (Alma, study) and reads-qc (SLURM E2E) — the **single shared blocker** for both; both run **warn-only** so the suites stay green. Looks like the local path lacking the terminal guard bug #5 added to the trace path.
7. **Trace-path output non-resolution** — when the **pipeline-monitor** finalizes a run (not the `/sync`/queue path), the run + per-sample artifacts land but the **run-scoped summary** row and the **per-read `readCount1`/`avgQuality1` merge** do not ingest (fastqc: 0 of N reads). Same class as bug #4. The summary-TSV and read-field checks **warn+skip** on a wholesale miss rather than red the suite, hard-asserting only when present.
