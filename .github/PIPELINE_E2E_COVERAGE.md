...
| **metaxpath** | — / ⚠️ | ✅ Alma (hard local + ✅ SLURM) | **Local (install path): hard gate, green** — `completes` + trace (real classification) + **taxonomy-content enforced** — ≥1 taxon from the curated `combined_report` (≥3 relaxed to ≥1: the human-decontaminated Gemma data classifies to ~1 dominant taxon; `SEQDESK_METAXPATH_EXPECT_TAXON` adds a per-organism check). **SLURM (install path): ✅ green — warn-only, manual-dispatch** (`scripts/metaxpath-slurm-leg.sh`, inline executor). Runs the full 13-step DAG on the cluster (36 tasks, 10 classification processes, 35 taxa, finalized via the scheduler). Reaching green needed three SeqDesk fixes: **(1)** the SLURM time limit is in **HOURS** — the inline wrapper writes `#SBATCH -t <N>:0:0`, so the old `60` meant 60 *hours* → PENDING forever with reason `(PartitionTimeLimit)`; set to `2`. **(2)** the inline executor skips `process.resourceLimits`, so a process's raw `cpus` hits the sbatch cgroup — metaxpath's `cpus={params.threads}=20` was rejected pre-flight in a `-c 2` job; cap with `-c 4` + `--config-json {"threads":4,…}`. **(3)** the pipeline-monitor false-completed the run after the 2-of-13 input-prep wave (stepMap holds only appeared steps); fixed by requiring all defined steps for SLURM runs (`completedSteps>=totalSteps`, scoped to SLURM so local runs keep their ingesting path), so it finalizes from the scheduler/exit marker. Warn-only because the inline job is single-node over the network FS; the local run remains the canonical hard gate. |
| **mag** | — / ⚠️ | ⚠️ Alma (local + ✅ SLURM) | MEGAHIT-only smoke (skips SPAdes/CONCOCT/Prokka/bin-QC/GTDB-Tk to fit the runner); assembly written back. **SLURM: ✅ green — warn-only, manual-dispatch** (`scripts/mag-slurm-leg.sh`, inline executor) — the same MEGAHIT smoke through the SLURM executor, reusing the metaxpath playbook: caps every process via nf-core's own `--max_cpus 4 --max_memory 40.GB` (the inline path skips `resourceLimits`), `-t 2:0:0`, and finalizes from the scheduler since the smoke skips steps. **TODO:** promote local to hard; full run needs GTDB staged (`gtdbDb` config) + in-app DB download |
