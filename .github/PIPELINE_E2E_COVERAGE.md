...
| reads-qc              | ⚠️     | ⚠️     | study  | `completes` gate (run `completed`, outputs retrievable)                      | —            | per-process `seqkit`/`python` conda (shared cacheDir like fastqc) | **wired warn-only** (SLURM E2E) — flushing the local `completed`→`running` flip; promote to hard once green |
...
| metaxpath             | —     | —     | study  | taxonomy/path results                                                        | —            | **MetaXpath DB bundle** (private pkg; **no GTDB** — mag-only) | preflight wired (Alma install E2E); **run wired (warn-only)** on the Gemma study, `completes`. **Install/enable now works** (run attempted on the runner; corrected order→**study** after the app rejected order). Pending first completed run |
...
- **reads-qc** — study target; per-process `seqkit`/`python` conda. **Now wired (warn-only)** in the SLURM E2E (a study-target `--pipeline-id reads-qc` step, local + SLURM, `completes` gate). The warn-only framing is deliberate: the real-data Gemma run surfaced a local-run `completed`→`running` re-sync flip (distinct from the queue-side bug #5 guard), and this step flushes out whether it still bites on synthetic data **without** reding the suite — promote to a hard step + a read-field/artifact spec (beyond `completes`) once green (`readCount`/`avgQuality` are now exposed by the run-GET select).
...
| metaxpath         | ✅ taxonomy/path (the point)   | **wired (warn-only)** — `run_gemma_warn metaxpath` on the `gemma-nanopore-metaxpath` **study** (`completes`). **Install/enable now works** — a run was attempted (order→study target fix applied after the app rejected order); pending first completed run (conda env + DB on the runner). **GTDB not needed** (mag-only) |
