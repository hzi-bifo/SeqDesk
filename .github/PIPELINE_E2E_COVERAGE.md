...
| submg | ✅ local (own wf) / — | 📋 planned | covered via its **own** workflow `pipeline-submg-e2e.yml` (source-boot app, **local** execution): registers a study on **ENA Test** (wwwdev), provisions a `submg` conda env (submg from source + Java + Webin-CLI jar), runs submg through the app and asserts the **sample ENA accession** is written back (ERS/SAMEA). **SLURM mode is n/a** — submg submits to ENA over the network, which the compute nodes can't reach, so it always runs local on a **GitHub-hosted runner** (like the sibling `ena-submission-e2e`). **Installed-app** coverage is not wired yet (would need a `submg` env + ENA egress on the install harness). Submits **samples only** by default for a reliable round-trip; read/assembly submission (heavier webin uploads) is opt-in + warn-only. Self-skips without `ENA_USERNAME`/`ENA_PWD` secrets. Caught that the runner targeted `submg submit` with underscore flags — current submg ships the submit command under **`submg-cli`** with kebab-case flags and selects the ENA test server via `--development-service 1` (it ignores `ENA_TEST_MODE`); the runner now resolves `submg-cli`, emits kebab flags, passes `--development-service`, tolerates local execution (`${SLURM_JOB_ID:-local}`), tab-splits the accessions file, and parses submg's `<timestamp>/…/reads_<NAME>/` report layout. Also fixed a `$queryRaw`-on-`pg_advisory_xact_lock()` void-deserialization bug that 500'd study registration before ENA was contacted. |
