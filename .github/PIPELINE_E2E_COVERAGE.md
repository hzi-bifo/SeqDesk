...
| **metaxpath** | — / ⚠️ | ✅ Alma (hard, local) | **Local (install path): hard gate, green** — `completes` + trace (real classification) + **taxonomy-content enforced** — ≥1 taxon from the curated `combined_report` (≥3 relaxed to ≥1: the human-decontaminated Gemma data classifies to ~1 dominant taxon; `SEQDESK_METAXPATH_EXPECT_TAXON` adds a per-organism check). **SLURM (install path): ⚠️ warn-only, manual-dispatch only.** metaxpath provably *runs* under the SLURM executor (run `27823876808`: 36 processes, exit 0, taxonomy reports produced), but it can't be a reliable gate: the inline job reads the DB over the network FS (slow, ~1.5–3 h) and, decisively, usually sits **PENDING for hours** on the shared cluster — *every* allocation tried pended (256/64/32 GB, 16/4/2 cores, 150/90 min limits, smaller subsets); only one daytime run found a free slot. Gated to manual dispatch (`subsample_reads`) so the nightly isn't bloated by pending; `scripts/metaxpath-slurm-leg.sh`. The local run above is the canonical metaxpath validation. |
