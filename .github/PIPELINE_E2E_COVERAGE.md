...
| metaxpath             | —     | —     | order  | taxonomy/path results                                                        | —            | **MetaXpath DB bundle** (private pkg; **no GTDB** — mag-only) | preflight wired (Alma install E2E); **metaxpath-on-Gemma run now wired (warn-only)** — order target, `completes`; profile version updated by the maintainer, pending first green on the runner |
...
| metaxpath         | ✅ taxonomy/path (the point)   | **wired (warn-only)** — `run_gemma_warn metaxpath` on `DEV-GEMMA-ONT-001` (order target, `completes`); profile version updated, pending first green (install + DB params file on disk + conda env on the runner). **GTDB not needed** (mag-only) |
...
Follow-ups: confirm the MetaxPath package install goes green on the runner (the profile's version was updated by the maintainer to clear the prior sub-second failure; the install-log tail is surfaced on failure in `install-dist.sh`) and pre-warm its conda cacheDir. The metaxpath-on-Gemma run is now **landed** (`metaxpath: { kind: "completes" }` in `WRITEBACK_SPEC` + an order-scoped `run_gemma_warn metaxpath` line, **warn-only**), pending its first green run; investigate the reads-qc local completed→running flip; enable study-demo-report on the profile to cover it; stage **GTDB for mag** (not metaxpath — metaxpath needs only its own DB bundle).
