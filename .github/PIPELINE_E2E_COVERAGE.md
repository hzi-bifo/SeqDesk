...
**SLURM-on-install — in progress.** Making the *installed* app submit a real `sbatch` job is mapped (from the config loader): install with `SEQDESK_EXEC_USE_SLURM` + conda path/env (persisted to `SiteSettings.pipelineExecution`), then boot with the runtime-only env the source-boot app uses (`SEQDESK_SLURM_INLINE_EXECUTOR=1` for the QOS=1 cap, `SEQDESK_PIPELINE_RUN_DIR`, `SEQDESK_PIPELINES_DIR`, `SEQDESK_CONDA_CACHE_DIR`, the shared conda env — `start.sh` inherits the parent env). **The one hard constraint:** the install's **data + run dirs must live on the shared cluster filesystem** (compute nodes can't see the runner-local install dir), so the reads have to be seeded onto `/net/broker`, not the local install dir. fastq-checksum is the first target. **Approach (refined):** do the install **inside the SLURM E2E workflow** — it already has the shared FS, the prebuilt conda env, `SLURM_SHARED_*`, and the inline executor — as a **warn-only** step that installs a built release (data/runs on `/net/broker`, SLURM exec configured), boots the *installed* app, and runs `fastq-checksum --skip-local` through it, asserting the SLURM writeback. This reuses the existing SLURM setup instead of duplicating it, and leaves the source-boot job untouched. Still to nail before it's first-try-green: how `install.sh` consumes a locally-built release (vs. the Alma npm-launcher + mock-release-server flow), and pointing the **pipelines dir** at the shared FS so the compute node can read it.
