...
import { buildMountPlan, carveOutDenies, describeMountPlan, mountPlanHash, renderBwrapArgs, renderSeatbeltProfile, validateMountPlan, type MountPlanInput } from "./mount-plan";
...
describe("mount plans", () => {
  ...
  it("renders a Seatbelt profile that denies the carved-out roots and every write outside the run", () => {
    ...
    expect(profile).toContain('(allow file-write* (literal "/dev/null") (subpath "/tmp") (subpath "/private/tmp") (subpath "/data/explore/runs/EXP-1--id-run1"))');
  });
