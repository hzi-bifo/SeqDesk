...
describe('syncPipelineRunForOperator (with trace file)', () => {
  ...
  it('demotes a falsely-completed inline-SLURM run with no exit marker while the sbatch job is still RUNNING', async () => {
    // Inline-SLURM metaxpath false-completion: a no-step-def run wrapped in one sbatch job was
    // finalized 'completed' from a trivially-100% trace after only its fast input-prep wave
    // (INPUT_CHECK + MV_FASTQ), before classification. The network-isolated node sends no weblog and
    // run.sh has not written its canonical exit marker yet, but squeue still reports the job RUNNING —
    // so the premature 'completed' must be demoted back to running until the marker appears.
    mocks.db.pipelineRun.findUnique.mockResolvedValue({
      ...traceRun,
      status: 'completed',
      completedAt: new Date('2026-03-03T11:00:00Z'),
      queueJobId: '6376', // numeric => inline-SLURM sbatch job id
    });
    mocks.getStepsForPipeline.mockReturnValue([]); // no package step defs => totalSteps 0
    mocks.inferPipelineExitCode.mockResolvedValue(null); // marker absent => run.sh not finished
    mocks.execFile.mockImplementation((file, _args, callback) => {
      if (file === 'squeue') callback(null, { stdout: 'RUNNING|\n', stderr: '' });
      else callback(new Error('no sacct'));
    });
    mocks.parseTraceFile.mockResolvedValue(
      trace({
        tasks: [
  ...
  it('keeps a genuinely-completed inline-SLURM run completed once its exit marker exists', async () => {
    // Over-reach guard: a real finished inline-SLURM run HAS its canonical exit marker, so even while
    // its sbatch job briefly lingers RUNNING it must STAY completed (no demotion).
    mocks.db.pipelineRun.findUnique.mockResolvedValue({
      ...traceRun,
      status: 'completed',
      completedAt: new Date('2026-03-03T11:00:00Z'),
    ...
    mocks.parseTraceFile.mockResolvedValue(
      trace({
        tasks: [{ process: 'METAX_PROFILE', status: 'COMPLETED', complete: new Date('2026-03-03T11:00:00Z') }],
        overallProgress: 100,
      })
    );
