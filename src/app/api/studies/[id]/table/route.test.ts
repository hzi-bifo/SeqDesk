...
describe("GET /api/studies/[id]/table", () => {
  ...
  it("builds identity + status + per-sample field columns (deduping core fields)", async () => {
    const res = await GET(req(), { params });
    expect(res.status).toBe(200);
    const body = await res.json();
    const labels = body.columns.map((c: { label: string }) => c.label);
    // identity + status + the visible per-sample field; the scientific_name field
    // is folded into Organism and the hidden field is dropped.
  ...
  it("maps facilityStatus to a known status and falls back to WAITING", async () => {
    const res = await GET(req(), { params });
    const body = await res.json();
    expect(body.rows[0].status).toBe("SEQUENCED");
    expect(body.rows[0].cells["f:collection_date"]).toBe("2024-01-01");
    expect(body.rows[0].cells._organism).toBe("Escherichia coli (taxid 562)");
    // invalid status -> WAITING; accession surfaced; empty checklist cell
    expect(body.rows[1].status).toBe("WAITING");
    expect(body.rows[1].cells._accession).toBe("SAMEA123");
    expect(body.rows[1].cells["f:collection_date"]).toBe("");
  });
  ...
  it("reports perStudy=true when dynamic-studies is on and the study has its own form", async () => {
    ...
    // the per-study schema was requested for this study
