...
function validateFieldValue(field: FormFieldDefinition | undefined, value: unknown): string | null {
  ...
  if (field.required && isMissingRequiredValue(field, value)) return "Required";
  ...
  if (v.minLength && strValue.length < v.minLength) return `Min ${v.minLength} characters`;
  if (v.maxLength && strValue.length > v.maxLength) return `Max ${v.maxLength} characters`;

  if (field.type === 'number') {
    const num = parseFloat(strValue);
    if (!isNaN(num)) {
      if (v.minValue !== undefined && num < v.minValue) return `Min value: ${v.minValue}`;
      if (v.maxValue !== undefined && num > v.maxValue) return `Max value: ${v.maxValue}`;
    }
  }
  ...
  if (v.pattern) {
    try {
      ...
      if (!regex.test(strValue)) return v.patternMessage || "Invalid format";
    } catch { /* skip */ }
  }
...
const EditableCell = React.memo(function EditableCell({
  ...
  const cellClassName = cn(
    ...
    "focus:ring-2 focus:ring-inset cursor-text",
    showError
    ...
    <div className="relative">
      <input
        ...
        onChange={(e) => {
          ...
          const error = e.target.value ? validateFieldValue(field as FormFieldDefinition, e.target.value) : null;
          setLocalValidationError(error);
          if (hasBlurred) {
            setContextValidationError(error);
          }
...
const FIELD_DEFINITIONS = {
  title: {
    id: "title", name: "title", label: "Study Title", type: "text" as const,
    required: true, visible: true, order: 1,
    helpText: "Choose a descriptive title that reflects the biological context of your study.",
    placeholder: "e.g., Soil microbiome survey 2024",
  },
  description: {
    id: "description", name: "description", label: "Description", type: "textarea" as const,
    required: false, visible: true, order: 2,
    helpText: "Provide additional context about your study objectives, methods, or any relevant information.",
    placeholder: "Describe the study objectives, methods, or any relevant context...",
  },
  checklistType: {
    id: "checklistType", name: "checklistType", label: "Environment Type", type: "select" as const,
    required: true, visible: true, order: 3,
    helpText: "Select the environment type that best matches where your samples were collected.",
  },
  samples: {
    id: "samples", name: "samples", label: "Sample Selection", type: "multiselect" as const,
    required: true, visible: true, order: 4,
    helpText: "Select samples from your orders to include in this study.",
  },
};
...
interface AvailableSample {
  ...
  order: { id: string; orderNumber: string; name: string | null; status: string };
...
interface StudyFormConfig {
  ...
  modules: { mixs: boolean; sampleAssociation: boolean; funding: boolean };
}
...
interface MixsField {
  ...
  group?: string;
  simpleValidation?: {
    minLength?: number; maxLength?: number; minValue?: number; maxValue?: number;
...
export default function EditStudyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [baseStudyMetadata, setBaseStudyMetadata] = useState<Record<string, unknown>>({});
  ...
  useEffect(() => {
    ...
    setStudyFieldValues((prev) => {
      ...
      for (const field of studyFields) {
        ...
        if (baseStudyMetadata[field.name] !== undefined) {
          next[field.name] = baseStudyMetadata[field.name];
    ...
    const fetchStudy = async () => {
      try {
        ...
        if (study.studyMetadata) {
          try {
            parsedMetadata = JSON.parse(study.studyMetadata);
          } catch {
            parsedMetadata = {};
          }
        }
        setBaseStudyMetadata(parsedMetadata);

        // Set selected sample IDs
        const sampleIds = study.samples.map(s => s.id);
        setSelectedSampleIds(sampleIds);

        // Initialize sample metadata from existing checklistData
        const metadataRows: SampleMetadataRow[] = study.samples.map(sample => {
          const row: SampleMetadataRow = {
            id: sample.id,
            sampleId: sample.sampleId,
          };
          if (sample.checklistData) {
            try {
              const data = JSON.parse(sample.checklistData);
        ...
        setSampleMetadata(metadataRows);
      } catch (err) {
    ...
    const fetchConfig = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setFormConfig({ perSampleFields: [], studyFields: [], groups: [], modules: { mixs: true, sampleAssociation: true, funding: false } });
        }
      } catch {
        setFormConfig({ perSampleFields: [], studyFields: [], groups: [], modules: { mixs: true, sampleAssociation: true, funding: false } });
      } finally {
  ...
  useEffect(() => {
    ...
    setSampleMetadata(prev => {
      ...
      return selectedSamples.map(sample => {
        ...
        const row: SampleMetadataRow = { id: sample.id, sampleId: sample.sampleId };
        if (sample.checklistData) {
          try {
            const data = JSON.parse(sample.checklistData);
            Object.entries(data).forEach(([key, value]) => { row[key] = value; });
          } catch { /* ignore */ }
        }
        return row;
      });
      ...
      for (let i = 0; i < mixsTemplate.fields.length; i++) {
        ...
        if (field.required) {
          requiredMixsFields.push({
            id: `_mixs_${field.name}`, name: `_mixs_${field.name}`, label: field.label,
            type: field.type, required: true, visible: true, order: i,
            helpText: field.helpText, placeholder: field.placeholder, example: field.example,
            options: field.options, units: field.units, group: field.group,
            simpleValidation: field.simpleValidation, source: 'mixs',
          });
        }
      }
      for (let i = 0; i < mixsTemplate.fields.length; i++) {
        ...
        if (!field.required && selectedMixsFields.has(field.name)) {
          optionalMixsFields.push({
            id: `_mixs_${field.name}`, name: `_mixs_${field.name}`, label: field.label,
            type: field.type, required: false, visible: true, order: i,
            helpText: field.helpText, placeholder: field.placeholder, example: field.example,
            options: field.options, units: field.units, group: field.group,
            simpleValidation: field.simpleValidation, source: 'mixs',
          });
  ...
  const nextStep = () => {
    ...
    if (currentStepId === "samples") {
      markTouched("samples");
      if (selectedSampleIds.length === 0) { setError("Please select at least one sample"); return; }
    }
    ...
    setSampleMetadata(old => {
      ...
      if (newData[rowIndex]) {
        if (typeof columnIdOrUpdates === "object") {
          // Multi-field update
          newData[rowIndex] = { ...newData[rowIndex], ...columnIdOrUpdates };
        } else {
          // Single field update
          newData[rowIndex] = { ...newData[rowIndex], [columnIdOrUpdates]: value };
        }
      }
      return newData;
    });
  ...
  const tableColumns = useMemo((): ColumnDef<SampleMetadataRow>[] => {
    const cols: ColumnDef<SampleMetadataRow>[] = [
      {
        id: "rowNumber", header: "#", size: 50,
        cell: ({ row }) => <div className="text-center text-muted-foreground font-medium text-sm">{row.index + 1}</div>,
      },
      {
    ...
    for (const field of allPerSampleFields) {
      ...
      cols.push({
        ...
        cell: cellType,
        meta: { field: field as FormFieldDefinition, editable: !isLoading, isMixsField },
      });
    }
  ...
  const buildStudyMetadataPayload = () => {
    const metadata: Record<string, unknown> = { ...baseStudyMetadata };
  ...
  const handleSubmit = async (skipWarnings = false) => {
    ...
    try {
      ...
      // Update sample associations and metadata
      if (formConfig?.modules.sampleAssociation) {
        ...
        if (!assignRes.ok) {
          throw new Error("Failed to update samples");
        }
      }
    ...
    return (
      <div className="flex h-full min-h-[420px] flex-col gap-4">
        ...
        {allPerSampleFields.length === 0 ? (
          <div className="flex min-h-0 flex-1 items-center justify-center rounded-lg bg-muted/30 py-8 text-center">
              ...
              <table className="min-w-full border-collapse bg-white text-sm">
                ...
                <tbody>
                  {metadataTable.getRowModel().rows.length === 0 ? (
                    <tr>
                      <td colSpan={tableColumns.length} className="px-4 py-8 text-center text-muted-foreground border-b">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="title" className="text-base flex items-center gap-2">
                ...
                {touched.title && isTitleValid && <CheckCircle2 className="h-4 w-4 text-green-500" />}
              </Label>
              <div className="relative">
                <Input
                  id="title"
            ...
            {adminOnlyStudyFields.length > 0 && (
              <div className="space-y-4 pt-6 border-t border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300">Facility Fields</h3>
                </div>
                <p className="text-xs text-slate-500">These fields are only visible to facility admins.</p>
                <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="space-y-6">
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="text-sm text-muted-foreground">Select the environment type that best matches your samples.</p>
                {touched.checklistType && isChecklistValid && (
                  <div className="flex items-center gap-1.5 text-green-600 text-sm">
                    <CheckCircle2 className="h-4 w-4" /><span>Selected</span>
                  </div>
                )}
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3" onFocus={() => setFocusedField(FIELD_DEFINITIONS.checklistType)}>
                {CHECKLIST_TYPES.map((checklist) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Icon className={cn("h-6 w-6 mb-2", isSelected ? "text-green-600" : "text-muted-foreground")} />
                      <p className={cn("font-medium text-sm", isSelected && "text-green-700")}>{checklist.name}</p>
            ...
            {isStudyChecklistTypeId(checklistType) && (
              <div className="border-t pt-6">
                <div className="mb-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Required fields are always included. Choose optional fields to add.</p>
                </div>
                ...
                {loadingMixs ? (
                  ...
                  <div className="space-y-6">
                        ...
                        <div>
                          ...
                          {filteredFields.length === 0 ? (
                            <div className="text-center py-6 bg-muted/30 rounded-lg">
                              <p className="text-sm text-muted-foreground">No fields match &quot;{mixsFieldSearch}&quot;</p>
                            </div>
                          ) : (
                                ...
                                return (
                                  ...
                                  >
                                    <div className={cn(
                                      ...
                                      isFieldSelected ? "border-emerald-500 bg-emerald-500" : "border-muted-foreground/30"
                                    )}>
                                      {isFieldSelected && <Check className="h-3 w-3 text-white" />}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                      <p className={cn("text-sm font-medium truncate", isFieldSelected && "text-emerald-800")}>{field.label}</p>
                    ...
                    <p className="text-xs text-muted-foreground">{selectedMixsFields.size} of {mixsTemplate.fields.length} fields selected</p>
                  </div>
                ) : (
                  <div className="text-center py-8 bg-muted/30 rounded-lg">
                    <p className="text-sm text-muted-foreground">No additional metadata fields available.</p>
                  </div>
                )}
              </div>
            )}
        ...
        return (
          <div className="flex h-full min-h-[360px] flex-col gap-4" onFocus={() => setFocusedField(FIELD_DEFINITIONS.samples)}>
            <div className="flex shrink-0 items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground">Select samples to include in this study.</p>
                {selectedSampleIds.length > 0 && (
                  <p className="text-sm text-green-600 mt-1">{selectedSampleIds.length} sample{selectedSampleIds.length !== 1 ? "s" : ""} selected</p>
                )}
              </div>
              <div className="flex items-center gap-2">
                {eligibleSamples.length > 0 && (
                  <Button type="button" variant="outline" size="sm" onClick={() => setSelectedSampleIds(eligibleSamples.map(s => s.id))}>
                    Select All
                  </Button>
                )}
                {selectedSampleIds.length > 0 && (
                  <Button type="button" variant="ghost" size="sm" onClick={() => setSelectedSampleIds([])}>Clear</Button>
                )}
              </div>
            </div>

            {loadingSamples ? (
              <div className="flex min-h-0 flex-1 items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
            ) : eligibleSamples.length === 0 ? (
              <div className="flex min-h-0 flex-1 flex-col items-center justify-center rounded-lg bg-muted/30 py-12 text-center">
                ...
                <p className="text-muted-foreground mb-2">No available samples</p>
                <p className="text-sm text-muted-foreground">Create an order and add samples first.</p>
                <Button className="mt-4" size="sm" asChild><Link href="/orders/new">Create Order</Link></Button>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground truncate">
                          ...
                          {isCurrentStudy && <span className="text-emerald-600 ml-2">(in this study)</span>}
                        </p>
                      </div>
              ...
              <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-lg font-semibold text-green-700">Ready to save changes</p>
                    <p className="text-sm text-green-600 mt-1">Click &quot;Save Changes&quot; to update your study.</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-amber-600 mt-1">Please go back and complete all required fields.</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between py-2 border-b border-border">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isTitleValid ? <CheckCircle2 className="h-4 w-4 text-green-500" /> : <AlertCircle className="h-4 w-4 text-amber-500" />}
                  </div>
                </div>
                ...
                {formConfig?.modules.mixs && (
                  <div className="flex items-center justify-between py-2 border-b border-border">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isChecklistValid ? <CheckCircle2 className="h-4 w-4 text-green-500" /> : <AlertCircle className="h-4 w-4 text-amber-500" />}
                    </div>
                  </div>
                )}
                ...
                {formConfig?.modules.sampleAssociation && (
                  <div className="flex items-center justify-between py-2 border-b border-border">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isSamplesValid ? <CheckCircle2 className="h-4 w-4 text-green-500" /> : <AlertCircle className="h-4 w-4 text-amber-500" />}
                    </div>
                  </div>
                )}
  ...
  if (loadingConfig || initialLoading) {
    return (
  ...
  return (
    <div className="flex h-[calc(100svh-3.5rem)] flex-col overflow-hidden p-4 pb-16 md:h-[calc(100svh-2.5rem)] md:p-8 md:pb-16">
      ...
      <div className="mb-8 flex shrink-0 items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold">Edit Study</h1>
