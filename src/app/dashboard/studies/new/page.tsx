...
const EditableCell = React.memo(function EditableCell({
  ...
  const onFocus = useCallback(() => {
    if (field) {
      // Get scroll position from table meta before state update
...
export default function NewStudyPage() {
  ...
  // Save scroll position whenever the table scrolls
  useEffect(() => {
  ...
  const STEPS: Step[] = useMemo(() => {
    ...
    // Add sample selection step only if enabled
  ...
  const updateSampleMetadata = useCallback((rowIndex: number, columnIdOrUpdates: string | Record<string, unknown>, value?: unknown) => {
    // Get the stored scroll position (more reliable than reading current position)
