...
const EditableCell = React.memo(function EditableCell({
  ...
  const onFocus = useCallback(() => {
    if (field) {
      const scrollLeft = table.options.meta?.getScrollPosition?.() ?? 0;
      ...
      requestAnimationFrame(() => {
        table.options.meta?.restoreScrollPosition?.(scrollLeft);
      });
    }
...
export default function EditStudyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const studyFields = useMemo(() => {
    const fields = (formConfig?.studyFields || [])
      .filter((field) => field.visible !== false && field.type !== "mixs");
  ...
  useEffect(() => {
    ...
    const handleScroll = () => { scrollPositionRef.current = scrollContainer.scrollLeft; };
  ...
  const updateSampleMetadata = useCallback((rowIndex: number, columnIdOrUpdates: string | Record<string, unknown>, value?: unknown) => {
    ...
    requestAnimationFrame(() => {
      const scrollContainer = tableScrollRef.current;
      if (scrollContainer && scrollLeft > 0) scrollContainer.scrollLeft = scrollLeft;
    });
  }, []);
  ...
  const metadataTable = useReactTable({
    ...
    meta: {
      ...
      restoreScrollPosition: (scrollLeft: number) => {
        const scrollContainer = tableScrollRef.current;
        if (scrollContainer && scrollLeft > 0) scrollContainer.scrollLeft = scrollLeft;
      },
    },
          ...
          <div className="space-y-4" onFocus={() => setFocusedField(FIELD_DEFINITIONS.samples)}>
            ...
            {loadingSamples ? (
              ...
              <div className="text-center py-12 bg-muted/30 rounded-lg">
                ...
                <Button className="mt-4" size="sm" asChild><Link href="/dashboard/orders/new">Create Order</Link></Button>
              </div>
            ) : (
