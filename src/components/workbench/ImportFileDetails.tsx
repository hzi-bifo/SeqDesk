...
export function ImportFileDetails({ file }: { file: ImportFileInfo }) {
  ...
  return <details className="rounded border bg-background p-2 text-xs">
    <summary className="cursor-pointer break-words">File details · {file.filename}{typeof file.bytes === "number" ? " · " + (file.bytes / 1024 ** 2).toFixed(1) + " MiB" : ""}{url ? " · " + url.hostname + " · " + url.protocol.slice(0, -1).toUpperCase() : ""}</summary>
