...
export function AddDataMenu({ scope, reportId, onBuilt, withAnalysis = true, label = "Add", variant = "default", className, onInsertBlock, outputs, blocks, disabled = false, triggerContainer, open: controlledOpen, onOpenChange }: AddDataMenuProps) {
  ...
  return <>
    ...
    <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-4xl"><DialogHeader className="pr-5"><DialogTitle>{pageMode ? "Add to page" : "Add data"}</DialogTitle><DialogDescription>{pageMode ? "Choose saved data to add a table, create a chart or reuse a finished figure." : "Bring saved data onto the canvas, or create a chart for the report page."} No pipeline or analysis runs when you add data.</DialogDescription></DialogHeader>
      ...
      {tab === "files" && <section aria-label="Your files" className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4"><div><h3 className="flex items-center gap-2 text-sm font-semibold"><FileUp className="h-4 w-4 text-teal-700" />Import a table</h3><p className="mt-1 text-sm text-muted-foreground">Upload CSV, TSV or Excel. Preview and map its columns before importing.</p></div><Button size="sm" variant="outline" onClick={() => setImporting(true)}>Choose a file</Button></div>{reportId && !available && (reportError ? <div role="alert" className="space-y-3 text-sm"><p>Could not load saved files.</p><Button size="sm" variant="outline" onClick={() => void retryReport()}>Retry loading saved files</Button></div> : <ExploreLoading variant="cards" label="Loading saved files…" />)}<div className="grid gap-3 sm:grid-cols-2">{tables.filter(table => table.kind === "external").map(table => tableCard(table))}</div></section>}
    </DialogContent></Dialog>
