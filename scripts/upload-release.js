...
function getTarballPackageVersion(tarballPath, expectedVersion) {
  ...
  try {
    ...
    throw new Error(
      `Invalid package.json in tarball: ${
        error instanceof Error ? error.message : "Unknown error"
      }`
    );
  }
...
async function uploadToBlob(filePath, blobPath, token) {
...
async function main() {
  ...
  if (!options.version) {
    console.error("Usage: node scripts/upload-release.js <version> [options]");
    console.error("");
    ...
    console.error(
      '  node scripts/upload-release.js 0.1.6 --title "Bug Fixes" --changelog "Fixed X" "Improved Y"'
    );
    process.exit(1);
  }
