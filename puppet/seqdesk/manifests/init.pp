...
# @param bashrc_source Optional source for ${user_home}/.bashrc (e.g. 'puppet:///modules/profile/seqdesk_bashrc').
# @param bash_profile_source Optional source for ${user_home}/.bash_profile.
# @param puppet_conf_path Optional path to puppet.conf to manage (e.g. /etc/puppetlabs/puppet/puppet.conf). Requires puppet_conf_source or puppet_conf_content.
# @param puppet_conf_source Optional source for puppet.conf file.
...
# @param ssh_keys Optional array of hashes for SSH key pairs. Each hash: 'path' => basename under .ssh (e.g. 'id_ed25519_deploy'), 'private_source' => puppet URI, 'public_source' => puppet URI (optional). Private key gets mode 0600, public 0644.
#
class seqdesk (
  ...
  if !empty($ensure_directories) {
    ...
    if !empty($mounts) {
      Class['seqdesk::mounts'] -> Class['seqdesk::directories']
    }
  }
  ...
  $optional_config = $bashrc_source != undef or $bash_profile_source != undef or
    ($puppet_conf_path != undef and ($puppet_conf_source != undef or $puppet_conf_content != undef)) or
