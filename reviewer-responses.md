...
Much of that effort went into automated testing. We now run an extensive suite on every change: unit tests of the core logic, user-interface tests that drive the application in a real browser, and hosted end-to-end tests that regularly run the analysis pipelines on a real Slurm server to confirm that the pipeline integration still works. The same continuous integration also installs the published software from a clean machine and confirms that it starts, with each step shown as a status badge on the repository (**https://github.com/hzi-bifo/SeqDesk**). This makes it far less likely that a future update silently breaks something, and it keeps the application stable as it evolves.
...
Thanks for pointing this out. SeqDesk now has proper versioned releases: each one is tagged automatically on GitHub with downloadable build artefacts and checksums, the current release is **v1.1.120** (the full release history is at **https://github.com/hzi-bifo/SeqDesk/releases**), and a plain-language changelog is at **https://seqdesk.org/changelog**. Versions are therefore now unambiguous.
...
Thank you for flagging this so precisely. Both problems no longer apply, because the Django dependency-and-settings layer that caused them no longer exists. Configuration is now a single JSON file, `settings.json`. It holds both the application settings and the runtime connection details (database URL, authentication secret, and so on) that previously had to be supplied as separate environment variables, so a working install no longer needs a `.env` file at all; environment variables are still read, but only as optional overrides. Dependencies are pinned and installed reproducibly, and the installer checks its prerequisites up front and stops on any problem with a specific, fixable message rather than failing silently. Every setting the file accepts is documented at **https://seqdesk.org/docs/configuration**, and the installation guide at **https://seqdesk.org/docs/installation**; both sit within a full documentation site (**https://seqdesk.org/docs**), and a rewritten README and contributor guide accompany them. Most importantly, the install instructions are now continuously tested: CI installs the published software from scratch (from the package registry and via the one-line installer) against a fresh database, then confirms that it is reachable over HTTP, applies its database migrations, and supports a real login, signing in as the seeded administrator and researcher accounts and reaching role-gated pages. This runs on Ubuntu and AlmaLinux, and `npm i -g seqdesk` is additionally verified on macOS.
...
Thank you for raising this, and for naming two specific functions. The codebase now carries **over 3,600 automated tests**, run on every change, with about 80% line coverage enforced as a CI gate and reported publicly on Codecov (**https://codecov.io/gh/hzi-bifo/SeqDesk**). Both functions you named are covered:
...
Keeping pace with the MIxS standard is handled by configuration, not code. Each checklist is a versioned definition derived from the GSC/ENA package, and an administrator can pull the latest set from within the app, previewing the added, removed, and changed fields before anything is applied. Updates are non-destructive and version-pinned: each study records the checklist version it was created against and keeps being validated against it, so a later revision never alters metadata already entered, and checklists withdrawn upstream are retained rather than deleted. A facility can point at its own checklist source instead of ours, and because the full set ships with every install, the platform also works with no registry at all.
...
The ready-to-run set includes the nf-core/mag workflow you attempted, FastQC, a read-cleaning pipeline, and the lightweight "simulate reads" test-data pipeline you mentioned; any group can add its own Nextflow pipeline using the same manifest. The design is also layered: the core application (orders, samples, studies, metadata, and ENA registration) installs and runs on its own. Running the pipelines additionally needs a workflow runtime (Nextflow, with Conda or containers providing each pipeline's tools); a scheduler such as Slurm is supported for cluster-scale execution but is not required, since pipelines also run locally. The installer can provision these components, and each pipeline lists its own requirements. These paths are now tested continuously: the nf-core/mag workflow you attempted, alongside the other ready-to-run pipelines, is run end to end on test data on every change, including a real Slurm run on self-hosted runners set up like a facility environment, and we have improved run logging and diagnostics.
...
Thank you, this is a fair request. You can evaluate it now with no installation at **https://demo.seqdesk.org**, a researcher dashboard pre-seeded with example orders, studies, and sample metadata, where browsing, editing forms, and the metadata-ingestion and registration workflow behave exactly as in the full application. The documentation gap is also addressed: there is a complete site at **https://seqdesk.org/docs**, with a full installation section (**https://seqdesk.org/docs/installation**) covering the recommended one-command install (`npm i -g seqdesk`), a one-line installer, installation from source, configuration, and unattended setup, so the blank page you encountered no longer applies. On Docker: it is no longer an installation route (native install needs only Node.js and PostgreSQL, which we found simpler and more portable than container orchestration); the manuscript's Docker reference is outdated and we will correct it. The supported paths are CI-verified on Ubuntu and AlmaLinux, and `npm i -g seqdesk` on macOS as well. A few actions (file downloads, real ENA submission, and launching pipelines) are disabled on the shared demo for safety; the full feature set is one command away locally, and we are glad to provide a fully featured instance on request. Within the application, an administrator can also load a dummy dataset of orders, samples, and their metadata, so the ingestion workflow can be exercised directly in a local install. Demo behaviour is documented at **https://seqdesk.org/docs/core-concepts/demo-mode**.

---

# Reviewer 3

*Since submission the software has been substantially reimplemented (from Django to Node.js/TypeScript, now named SeqDesk; see Reviewer 1, Q1). Several points concern the framing of the manuscript itself; we address each and have revised the article where indicated.*
...
Thank you, and we agree. SeqDesk is built specifically around the sequencing-facility workflow (researchers submit sequencing orders, the facility manages samples and links sequencing files, optional analysis pipelines run, and studies and samples are brokered to ENA), rather than as a general laboratory information management system. We will revise the first half of the manuscript to state this scope explicitly and to remove the general-LIMS impression.
...
Thank you, and yes on both counts. Facilities can customise the platform without writing code: a built-in form builder lets administrators add center-specific fields and metadata to the order, sample, and study forms (including MIxS checklists and typed field types) and configure which modules and sequencing technologies are offered. Pipelines are not limited to nf-core/mag either; the platform runs any Nextflow pipeline described by a manifest, the shipped set already includes others (for example a read-cleaning pipeline that wraps nf-core/detaxizer, plus locally bundled workflows), and facilities can add their own. These are documented at **https://seqdesk.org/docs/administration/form-builders** and **https://seqdesk.org/docs/pipelines/adding-pipelines**, and we will expand the manuscript to describe them.
...
Thank you for the detailed account; it pinpoints exactly the failure modes of the old Django setup. That entire install path no longer exists: there is no `setup.sh`, no `requirements.txt`, and no Django, so those exact failures (the missing `django-environ`, the follow-on Django errors) cannot recur. SeqDesk now installs with a single command (`npm i -g seqdesk`): it checks prerequisites up front (Node.js 18+, npm, and a reachable PostgreSQL) and, on any problem, stops with a specific, fixable message (for example `Node.js 18+ is required (found v16…)`, `PostgreSQL refused the connection (ECONNREFUSED)`, or `The PostgreSQL database does not exist`) rather than failing silently, then downloads the release, installs dependencies, applies migrations, and seeds data, logging everything. The package-manager, one-line, and source installs are verified end to end by CI on Ubuntu, AlmaLinux, and macOS, through to a successful administrator login, so the manual dependency-patching you had to attempt should no longer be necessary. Dedicated installation and configuration docs are at **https://seqdesk.org/docs/installation** and **https://seqdesk.org/docs/configuration**. We could not reproduce a general install failure and would welcome the specific error you hit; for evaluation with no setup at all, **https://demo.seqdesk.org** needs no installation.
...
Thank you for raising this, and two clarifications. First, the public demo deliberately hides or disables parts of the administrative configuration, including some sequencing-center-specific setup, because it is a shared instance that resets periodically; those capabilities are present in a normal installation, and we are glad to provide a fully configured instance. Second, Excel import is in fact supported, not only export: samples can be imported into an order from an Excel sheet, with the columns mapped to the configured form fields and validated before the rows are accepted, and a matching template can be downloaded to fill in, and this import path is covered by tests at both the parsing and field-mapping level and the browser level. We will revise the article so that the described features match what a default installation provides.
